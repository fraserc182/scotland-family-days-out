...
export default function ScotlandDaysOut() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* require may be unavailable in some bundlers */
    }
        ...
        marker.bindPopup(
          `<div class="map-popup">
            ...
            <div class="map-popup-body">
              <div class="map-popup-location">📍 ${item.location}</div>
              <div class="map-popup-price">${item.price}</div>
              <div class="map-popup-cta">👉 Click to view details</div>
            </div>
          </div>`
        );
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (selected && target.classList.contains('modal-backdrop')) {
        setSelected(null);
      }
    };
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-blue-100 mt-2 text-sm md:text-base">
                Discover amazing activities for the whole family
              </p>
            </div>
            <div className="text-right flex-shrink-0 flex flex-col items-end gap-3">
              ...
              <Link
                href="/submit"
                className="px-4 py-2 bg-white text-blue-600 font-semibold rounded-lg hover:bg-blue-50 transition text-sm"
              >
                + Submit Activity
              </Link>
            </div>
      ...
      <div className="p-4 md:p-8">
        <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-[320px_1fr] gap-6">
          <aside className="bg-white p-6 rounded-2xl shadow-lg border border-cyan-100">
            ...
            <input
              ...
              className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200"
            />
            ...
            <div className="mt-6 pt-6 border-t border-slate-200">
              ...
              <div className="mt-3 flex gap-2 flex-wrap">
                {[
                  ...
                  <button
                    key={key}
                    onClick={() => setFilterCost(key)}
                    className={`filter-btn ${
                      filterCost === key ? 'filter-btn-active' : 'filter-btn-inactive'
                    }`}
                  >
                    {label}
                  </button>
            ...
            <div className="mt-6 pt-6 border-t border-slate-200">
              ...
              <div className="mt-3 flex gap-2">
                {[
                  ...
                  <button
                    key={key}
                    onClick={() => setFilterWeather(key)}
                    className={`px-3 py-2 text-sm rounded-full border transition-colors font-medium ${
                      filterWeather === key
            ...
            <div className="mt-6 pt-6 border-t border-slate-200 space-y-3">
              ...
              <label className="flex items-center gap-3 text-sm cursor-pointer hover:bg-red-50 p-2 rounded-lg transition">
                ...
                <span className="font-medium text-slate-700">
                  ❤️ Favorites {favourites.length > 0 && `(${favourites.length})`}
                </span>
              </label>
            </div>
            ...
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {filtered.map((item) => (
                  <article
                    key={item.id}
                    className="card-hover bg-white rounded-2xl p-5 shadow-md border border-cyan-100 flex flex-col overflow-hidden"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <button
                        ...
                        className="text-2xl flex-shrink-0 transition hover:scale-110"
                      >
        ...
        {selected && (
          <div className="modal-backdrop fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[9999]">
            <div
              className="bg-white rounded-3xl w-full shadow-2xl max-h-[90vh] overflow-y-auto"
            ...
            >
              <div className="header-gradient px-6 py-8 flex justify-between items-start gap-4">
                ...
                <button
                  ...
                  className="text-white text-2xl hover:opacity-80 transition flex-shrink-0"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="p-6">
                <div className="flex gap-2 mb-4 flex-wrap items-center">
                  ...
                  <button
                    ...
                    className="ml-auto text-2xl transition hover:scale-110 flex-shrink-0"
                  >
